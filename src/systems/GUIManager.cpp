...
void GUIManager::init(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    /*GUIManager *gui_manager = this;
    ctx.text_width = std::function<int(mu_Font, const char*, int)>{
    [gui_manager](mu_Font font, const char *text, int len) -> int {
        if (len == -1)
        {
            len = strlen(text);
        }
        return gui_manager->r_get_text_width(text, len);
    }
};*/
